#!/usr/bin/env python3
"""
LLM‑CER pipeline with anti‑transitivity hierarchical merging.
  1. LSH blocking
  2. LLM separation (whole blocks, skip singletons) + MDG
  3. Hierarchical merging with anti‑transitivity guard
"""

import ast, csv, math, os, re, sys, time
...
MAX_CLUSTERS_PER_PROMPT = 30          # never show more than this many reps at once
MERGE_MAX_ROUNDS = 15

REQUESTS_PER_MINUTE = 30

COLUMNS_TO_USE = ['brand', 'title', 'description', 'price', 'priceCurrency']
...
def load_data_and_vectorize(data_path):
    ...
    try: data = pd.read_csv(data_path, encoding='utf-8')
    except: data = pd.read_csv(data_path, encoding='utf-8-sig')
...
def _lexical_rescue(data,i,j):
    ...
    mi = _model_tokens(ti); mj = _model_tokens(tj)
    shared = mi&mj
    if bi and bj and bi==bj and shared: return True
    if shared and _title_overlap_ok(ti,tj): return True
    return False
...
def lsh_block(vectors, data):
    ...
    for members in brand_groups.values():
        ...
        for a_pos,a in enumerate(members):
            ma = _model_tokens(str(data.iloc[a].get('title','')))
            for b in members[a_pos+1:]:
                mb = _model_tokens(str(data.iloc[b].get('title','')))
                if len(ma&mb)>=1 or _title_overlap_ok(str(data.iloc[a].get('title','')),
...
def merge_prompt(rep_indices, df):
    ...
    prompt = (
        ...
        "Look at brand, model numbers, and title similarity.\n"
...
def sanitize_merged_clusters(clusters, df):
    """Split any merged cluster with multiple brands and no shared model tokens."""
    clean = []
    for cluster in clusters:
        ...
        brand_groups = defaultdict(list)
        for idx in cluster:
            brand = str(df.iloc[idx].get('brand', '')).strip().lower()
            if brand in {'nan', 'none', 'missing', ''}:
                brand = None
            brand_groups[brand].append(idx)
        ...
        for brand, indices in brand_groups.items():
            tokens = set()
            for idx in indices:
                t = str(df.iloc[idx].get('title', ''))
                tokens.update(_model_tokens(t))
            model_sets[brand] = tokens
        brands = list(model_sets.keys())
        must_split = False
        for i in range(len(brands)):
            for j in range(i+1, len(brands)):
                if not (model_sets[brands[i]] & model_sets[brands[j]]):
                    must_split = True
                    break
            if must_split:
                break
        if must_split:
            for indices in brand_groups.values():
                clean.append(sorted(set(indices)))
        else:
            clean.append(cluster)
    return clean
...
def hierarchical_merge_antitrans(initial_clusters, vectors, df, sim_matrix, id_map):
    """
    Anti‑transitivity aware hierarchical merge.
    Returns (final_clusters, total_calls, total_in_tok, total_out_tok)
    """
    ...
    while round_no < MERGE_MAX_ROUNDS:
        round_no += 1
        # compute centroids and build candidate graph
        centroids = [centroid_of(c, vectors) for c in clusters]
        n = len(clusters)
        # find similar pairs (candidate edges)
        edges = []
        for i in range(n):
        ...
        # maintain anti‑transitivity: pairs that were previously decided separate
        if not hasattr(hierarchical_merge_antitrans, "cannot_merge"):
            hierarchical_merge_antitrans.cannot_merge = set()

        # process each component
        new_clusters_indices = list(range(n))  # mapping from old index to new cluster index (after merges)
        merge_happened = False

        for comp in components:
            # remove edges forbidden by anti‑transitivity
            allowed_edges = [(i,j) for i in comp for j in comp if i<j and (i,j) not in hierarchical_merge_antitrans.cannot_merge and (j,i) not in hierarchical_merge_antitrans.cannot_merge]
            if not allowed_edges:
                continue

            # create sub-graph of allowed edges
            sub_graph = defaultdict(set)
            for i,j in allowed_edges:
                sub_graph[i].add(j)
                sub_graph[j].add(i)

            # split component into manageable groups (≤ MAX_CLUSTERS_PER_PROMPT)
            sub_visited = set()
            groups = []
            for node in comp:
                if node in sub_visited:
                    continue
                # BFS to collect up to MAX_CLUSTERS_PER_PROMPT nodes
                group = []
                queue = [node]
            ...
            for group in groups:
                ...
                # map LLM group indices back to global cluster indices
                global_merge_groups = []
                ...
                for global_group in global_merge_groups:
                    if len(global_group) == 1:
                        ...
                        # we'll assign the new cluster to the first index as a temporary ID
                        merged = sorted(set(merged))
                        new_clusters[global_group[0]] = merged
                        for other in global_group[1:]:
                            new_clusters[other] = None  # mark as removed

                # update anti‑transitivity: any pair not merged together is recorded
...
def main():
    ...
    data_path = os.getenv("DATASET_PATH", "sample_dataset_1.csv")
    gt_path = os.getenv("GROUND_TRUTH_PATH", "sample_dataset_1_gt.csv")
    ...
    safe_print("Step 2 – LLM separation + MDG (skipping size‑1 blocks)")
    all_sets = []
    ...
    safe_print("Step 3 – LLM hierarchical merging (anti‑transitivity)")
    initial_clusters = []
    for s in all_sets:

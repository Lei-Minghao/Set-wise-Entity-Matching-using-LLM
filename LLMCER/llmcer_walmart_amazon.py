#!/usr/bin/env python3
"""
LLM‑CER pipeline with conservative merge_2 + round limitations (NO chunking).
Tailored for Walmart‑Amazon product matching dataset (string IDs like walmart_53, amazon_10692).
  1. LSH blocking
  2. LLM separation + MDG (whole block processing, no chunking)
  3. Optimised merge_2 (batched LLM calls, only confirmed pairs merged) – iterative with round limit
"""
...
# Optimisation parameters for merge_2 (batched calls)
...
# Columns used for product representation
COLUMNS_TO_USE = ['title', 'category', 'brand', 'modelno', 'price', 'name', 'desc']
...
def separation_prompt(indices, df):
    # Build the text representation of each record (same as before)
    lines = build_record_lines(indices, df)
    
    # Fields available for matching (from the global COLUMNS_TO_USE)
...
def llm_separate_block(block_indices, df, sim_matrix, id_map):
    """Process a block as a whole (no chunking)."""
...
# ======================== CONSERVATIVE MERGE_2 (BATCHED, OPTIMISED) ========================
PRE_PROMPT_MERGE = (
    "You are an expert in product entity resolution. "
    "Two candidate clusters are shown below. Each cluster already contains product records that refer to the same product. "
    "Decide whether the two clusters should be merged into a single cluster (i.e., they represent the same product). "
    "Consider brand, model number, title, price, and description. "
...
def iterative_merge(initial_clusters, simi_matrix, df, block_threshold, merge_threshold, max_rounds):
    """
    Apply merge_2 repeatedly until no change or max_rounds reached.
    Returns (final_clusters, total_calls, total_in, total_out, total_time)
    """
...
def main():
    ...
    data_path = os.getenv("DATASET_PATH", "sample_walmart_amazon.csv")
    gt_path = os.getenv("GROUND_TRUTH_PATH", "sample_walmart_amazon_gt.csv")
